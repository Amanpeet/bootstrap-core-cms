...
if(isset($_POST["submit"])) {
  ...
  if(empty($_POST["username"]) || empty($_POST["password"])) {
    ...
    //$password = md5($password); // deprecated
    ...
    $sql = "SELECT `username`, `role` FROM admins WHERE `username`='$username' AND `password`=SHA2('$password',256) UNION SELECT `username`, `role` FROM users WHERE `username`='$username' AND `password`=SHA2('$password',256) LIMIT 1";
