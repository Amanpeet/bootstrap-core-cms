    ...
    if(isset($_POST['addmon'])) {
      ...
      if( !$error ){
        ...
        if( mysqli_num_rows($ex_query) > 0 ){
          ...
          $insert_sql = "INSERT INTO admins(`username`, `password`, `phone`, `email`, `role`) VALUES( '$username', SHA2('$password',256), '$phone', '$email', '$role' ) ";
  ...
  <div id="response" class="h5 pt-3 text-danger">
    ...
    if(isset($_POST['updatemon'])) {
      ...
      if( !$error ){
        ...
        $update = "UPDATE admins SET `password` = SHA2('$password',256), `phone` = '$phone', `email` = '$email', `role` = '$role' WHERE `uid` = '$id' ";
