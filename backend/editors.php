...
<div class="p-3 border-bottom">
  <h3> <strong>Editor Profiles</strong> </h3>
  <p>Click on <strong>View</strong> to view or edit the Editor.</p>
  <a class="btn btn-primary" href="editors-single.php?add=1">+ Add Editor</a>
</div>
...
<div class="p-2">
  ...
  <table class="table table-striped table-bordered table-responsive-md">
    ...
    <tbody>
      <?php
      $select = "SELECT * FROM users WHERE `role` = 'editor' ORDER BY `uid` DESC LIMIT $items_per_page OFFSET $start_from";
      ...
      while($fetch = mysqli_fetch_array($quer)){ ?>
        <tr>
          ...
          <td>
            <a class="btn btn-dark btn-sm" href="editors-single.php?view=<?php echo $fetch['uid']; ?>">View</a>
            <?php if($admin || $editor){ ?>
              <a class="btn btn-primary btn-sm" href="editors-single.php?edit=<?php echo $fetch['uid']; ?>">Edit</a>
              <a class="btn btn-danger btn-sm" href="editors-single.php?del=<?php echo $fetch['uid']; ?>">Delete</a>
            <?php } ?>
          </td>
        </tr>
      <?php } ?>
  ...
  <nav aria-label="Page navigation example">
    <ul class="pagination">
      <?php
        //pagination part 2
        $sql = "SELECT COUNT(uid) AS countx FROM users WHERE `role` = 'editor'";
